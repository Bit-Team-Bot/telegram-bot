...
echo "🔧 Fixing Telegram Bot Database Schema..."

# Get the directory of this script
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(dirname "$SCRIPT_DIR")"
DB_PATH="$PROJECT_ROOT/telegram_bot.db"
...
# sqlite3 "$DB_PATH" "UPDATE users SET is_superadmin = 1, role = 'superadmin' WHERE telegram_id = 'YOUR_TELEGRAM_ID';"
...
EOF
    ...
    echo "✅ Database schema fixed with SQLite"
fi
